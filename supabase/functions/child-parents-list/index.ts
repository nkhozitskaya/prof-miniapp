// Supabase Edge Function: list parents linked to current child.
...
Deno.serve(async (req) => {
  ...
  const { data: links, error: linksError } = await supabase
    ...
    .select('parent_user_id, status, created_at')
    .eq('child_user_id', payload.sub)
    .eq('status', 'active')
    .order('created_at', { ascending: false })

  if (linksError) return new Response(JSON.stringify({ error: linksError.message }), { status: 500, headers: { ...cors, 'Content-Type': 'application/json' } })

  const parentIds = (links ?? []).map((l) => l.parent_user_id as string)
  if (parentIds.length === 0) return new Response(JSON.stringify([]), { headers: { ...cors, 'Content-Type': 'application/json' } })
  ...
  if (parentsError) return new Response(JSON.stringify({ error: parentsError.message }), { status: 500, headers: { ...cors, 'Content-Type': 'application/json' } })

  const byId = new Map((parents ?? []).map((p) => [p.id as string, p]))
  const list = parentIds.map((id) => byId.get(id)).filter(Boolean)
