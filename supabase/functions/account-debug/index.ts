// Debug helper: check current account rows in users + user_profiles.
...
Deno.serve(async (req) => {
  ...
  const [u, p] = await Promise.all([
    supabase.from('users').select('id, telegram_user_id, first_name, last_name, username, created_at').eq('id', userId).maybeSingle(),
    supabase.from('user_profiles').select('user_id, display_name, age, phone, email, role, created_at, updated_at').eq('user_id', userId).maybeSingle(),
  ])

  return new Response(
    JSON.stringify({
      token_user_id: userId,
      users_row: u.data ?? null,
      users_error: u.error?.message ?? null,
      profile_row: p.data ?? null,
      profile_error: p.error?.message ?? null,
    }),
    { headers: { ...cors, 'Content-Type': 'application/json' } },
  )
})
