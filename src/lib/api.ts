/**
 * API: Telegram initData, вход по телефону, диагностика.
 * Base URL: VITE_API_URL (Supabase project URL).
 */
...
export async function authPhone(payload: {
  phone: string
  first_name?: string
  last_name?: string
  age?: number
}): Promise<AuthResponse> {
  const res = await fetch(`${BASE}/functions/v1/phone-auth`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
